...
#pragma warning restore 4014
    ...
    [Fact]
    public virtual async Task Should_Handle_By_Cookie_AuthenticationScheme_For_TiknasAuthorizationException_For_Void_Return_Value()
    {
...
#pragma warning restore 4014
    ...
    [Fact]
    public virtual async Task Should_Handle_By_JwtBearer_AuthenticationScheme_For_TiknasAuthorizationException_For_Void_Return_Value()
    {
        var result = await GetResponseAsync("/ExceptionHandling/ExceptionTestPage?handler=TiknasAuthorizationException", HttpStatusCode.Unauthorized);
...
#pragma warning restore 4014
    ...
    public async Task Should_Handle_Exception_On_Uow_SaveChangeAsync()
    {
        _fakeRequiredService.Claims.AddRange(new[]
        {
            new Claim(TiknasClaimTypes.UserId, Guid.Empty.ToString())
        });
