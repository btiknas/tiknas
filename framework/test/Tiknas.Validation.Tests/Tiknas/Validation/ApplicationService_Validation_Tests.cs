...
public class ApplicationService_Validation_Tests : TiknasIntegratedTest<ApplicationService_Validation_Tests.TestModule>
{
    ...
    {
        await Assert.ThrowsAsync<TiknasValidationException>(async () => await _myAppService.MyMethod(new MyMethodInput())); //MyStringValue is not supplied!
        await Assert.ThrowsAsync<TiknasValidationException>(async () => await _myAppService.MyMethod(new MyMethodInput { MyStringValue = "a" })); //MyStringValue's min length should be 3!
    }
    ...
    {
        await Assert.ThrowsAsync<TiknasValidationException>(async () =>
    ...
    //TODO: Create a Tiknas.Ddd.Application.Contracts.Tests project and move this to there and remove Tiknas.Ddd.Application.Contracts dependency from this project.
    [Fact]
    public async Task LimitedResultRequestDto_Should_Throw_Exception_For_Requests_More_Than_MaxMaxResultCount()
    {
        var exception = await Assert.ThrowsAsync<TiknasValidationException>(async () =>
        {
    ...
    [DependsOn(typeof(TiknasValidationModule))]
