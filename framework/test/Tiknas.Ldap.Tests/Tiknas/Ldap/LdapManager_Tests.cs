...
/// <summary>
/// docker run --name ldap -d --env LDAP_ORGANISATION="tiknas" --env LDAP_DOMAIN="tiknas.io" --env LDAP_ADMIN_PASSWORD="123qwe" -p 389:389 -p 636:639 osixia/openldap
/// </summary>
public class LdapManager_Tests : TiknasIntegratedTest<TiknasLdapTestModule>
{
    ...
    {
        (await _ldapManager.AuthenticateAsync("cn=admin,dc=tiknas,dc=io", "123qwe")).ShouldBe(true);
        (await _ldapManager.AuthenticateAsync("cn=tiknas,dc=tiknas,dc=io", "123123")).ShouldBe(false);
