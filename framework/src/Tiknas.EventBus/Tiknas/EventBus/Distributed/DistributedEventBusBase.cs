...
public abstract class DistributedEventBusBase : EventBusBase, IDistributedEventBus, ISupportsEventBoxes
{
    ...
    protected TiknasDistributedEventBusOptions TiknasDistributedEventBusOptions { get; }
        ...
        eventHandlerInvoker)
    {
        ...
        Clock = clock;
        TiknasDistributedEventBusOptions = tiknasDistributedEventBusOptions.Value;
    ...
    protected virtual async Task<bool> AddToOutboxAsync(Type eventType, object eventData)
    {
        ...
        foreach (var outboxConfig in TiknasDistributedEventBusOptions.Outboxes.Values.OrderBy(x => x.Selector is null))
        {
    ...
    {
        if (TiknasDistributedEventBusOptions.Inboxes.Count <= 0)
        {
            return false;
        }
        ...
        {
            foreach (var inboxConfig in TiknasDistributedEventBusOptions.Inboxes.Values.OrderBy(x => x.EventSelector is null))
            {
