...
using Tiknas.AspNetCore.Components;
using Tiknas.Localization;
using Tiknas.Authorization;
using Tiknas.BlazoriseUI.Components;
using Tiknas.BlazoriseUI.Components.ObjectExtending;
using Tiknas.ObjectExtending.Modularity;
using Tiknas.ObjectExtending;
using Tiknas.AspNetCore.Components.Web.Extensibility.EntityActions;
using Tiknas.AspNetCore.Components.Web.Extensibility.TableColumns;

namespace Tiknas.BlazoriseUI;

public abstract class TiknasCrudPageBase<
        TAppService,
        TEntityDto,
        TKey>
    : TiknasCrudPageBase<
    ...
    where TUpdateViewModel : class, new()
{
    ...
    [Inject] protected IStringLocalizer<TiknasUiResource> UiLocalizer { get; set; } = default!;
    [Inject] public ITiknasEnumLocalizer TiknasEnumLocalizer { get; set; } = default!;
    ...
    /// Throws <see cref="TiknasAuthorizationException"/> if given policy was not granted for the current user.
    ///
                ...
                else
                {
                    ...
                    if (propertyInfo.Type.IsEnum)
                    {
                        column.ValueConverter = (val) =>
                            TiknasEnumLocalizer.GetString(propertyInfo.Type, val.As<ExtensibleObject>().ExtraProperties[propertyInfo.Name]!, new IStringLocalizer?[]{ StringLocalizerFactory.CreateDefaultOrNull() });
                    }
