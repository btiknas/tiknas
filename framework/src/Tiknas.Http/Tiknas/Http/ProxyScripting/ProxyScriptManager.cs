...
public class ProxyScriptManager : IProxyScriptManager, ITransientDependency
{
    ...
    private readonly TiknasApiProxyScriptingOptions _options;
    ...
    public ProxyScriptManager(
        ...
        IOptions<TiknasApiProxyScriptingOptions> options)
    {
    ...
    private string CreateScript(ProxyScriptingModel scriptingModel)
    {
        ...
        {
            throw new TiknasException($"Could not find a proxy script generator with given name: {scriptingModel.GeneratorType}");
        }
