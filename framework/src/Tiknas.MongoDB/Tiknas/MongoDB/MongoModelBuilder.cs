...
public class MongoModelBuilder : IMongoModelBuilder
{
    ...
    public virtual MongoDbContextModel Build(TiknasMongoDbContext dbContext)
    {
        lock (SyncObj)
        {
            var useTiknasClockHandleDateTime = dbContext.LazyServiceProvider.LazyGetRequiredService<IOptions<TiknasMongoDbOptions>>().Value.UseTiknasClockHandleDateTime;
            ...
            foreach (var entityModel in entityModels.Values)
            {
                ...
                if (useTiknasClockHandleDateTime)
                {
                    ...
                    dateTimePropertyInfos.ForEach(property =>
                    {
                        ...
                        {
                            map.MapProperty(property.Name).SetSerializer(new NullableSerializer<DateTime>().WithSerializer(new TiknasMongoDbDateTimeSerializer(GetDateTimeKind(dbContext), disableDateTimeNormalization)));
                        }
                        else
                        {
                            map.MapProperty(property.Name).SetSerializer(new TiknasMongoDbDateTimeSerializer(GetDateTimeKind(dbContext), disableDateTimeNormalization));
                        }
                    });
                }
            ...
            foreach (var baseClass in baseClasses)
            {
                if (!BsonClassMap.IsClassMapRegistered(baseClass))
                {
                    ...
                    map.ConfigureTiknasConventions();
                    ...
                    if (useTiknasClockHandleDateTime)
                    {
                        ...
                        dateTimePropertyInfos.ForEach(property =>
                        {
                            if (property.PropertyType == typeof(DateTime?))
                            {
                                map.MapProperty(property.Name).SetSerializer(new NullableSerializer<DateTime>().WithSerializer(new TiknasMongoDbDateTimeSerializer(GetDateTimeKind(dbContext), false)));
                            }
                            else
                            {
                                map.MapProperty(property.Name).SetSerializer(new TiknasMongoDbDateTimeSerializer(GetDateTimeKind(dbContext), false));
                            }
                        });
                    }
    ...
    private DateTimeKind GetDateTimeKind(TiknasMongoDbContext dbContext)
    {
        return dbContext.LazyServiceProvider.LazyGetRequiredService<IOptions<TiknasClockOptions>>().Value.Kind;
    }
    ...
    protected virtual void CreateCollectionIfNotExists(TiknasMongoDbContext dbContext, string collectionName, CreateCollectionOptions createCollectionOptions)
    {
    ...
    protected virtual void CreateCollectionIndexes(TiknasMongoDbContext dbContext, string collectionName, Action<IMongoIndexManager<BsonDocument>>? indexesAction = null)
    {
