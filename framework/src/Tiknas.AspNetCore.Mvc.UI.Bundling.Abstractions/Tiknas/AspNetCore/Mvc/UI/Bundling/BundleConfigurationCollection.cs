...
public class BundleConfigurationCollection
{
    ...
    /// Throws <see cref="TiknasException"/> if there is already a bundle with the same name.
    /// </summary>
        ...
        Action<BundleConfiguration>? configureAction = null)
    {
        ...
        {
            throw new TiknasException($"There is already a bundle added with given {nameof(bundleName)}: {bundleName}");
        }
    ...
    public BundleConfiguration Get([NotNull] string bundleName)
    {
        ...
        if (!_bundles.TryGetValue(bundleName, out var bundle))
        {
            throw new TiknasException($"There is no bundle with given {nameof(bundleName)}: {bundleName}");
        }
