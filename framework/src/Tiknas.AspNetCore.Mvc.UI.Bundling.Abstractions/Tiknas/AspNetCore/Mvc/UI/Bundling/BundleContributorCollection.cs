...
public class BundleContributorCollection
{
    ...
    private void AddInstanceToContributors(Type contributorType)
    {
        ...
        {
            throw new TiknasException($"Given {nameof(contributorType)} ({contributorType.AssemblyQualifiedName}) should implement the {typeof(IBundleContributor).AssemblyQualifiedName} interface!");
        }
        ...
        {
            throw new TiknasException($"Can not instantiate {contributorType.AssemblyQualifiedName}", ex);
        }
    }
}
