...
{
    protected TiknasHangfireBackgroundJobServer BackgroundJobServer { get; set; } = default!;
    ...
    {
        BackgroundJobServer = ServiceProvider.GetRequiredService<TiknasHangfireBackgroundJobServer>();
    }
    ...
    public async override Task AddAsync(IBackgroundWorker worker, CancellationToken cancellationToken = default)
    {
        switch (worker)
        {
            ...
            case AsyncPeriodicBackgroundWorkerBase or PeriodicBackgroundWorkerBase:
            {
                ...
                var period = worker is AsyncPeriodicBackgroundWorkerBase ? ((TiknasAsyncTimer?)timer)?.Period : ((TiknasTimer?)timer)?.Period;
