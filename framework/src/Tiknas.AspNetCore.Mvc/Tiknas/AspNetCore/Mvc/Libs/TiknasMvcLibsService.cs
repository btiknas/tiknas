...
namespace Tiknas.AspNetCore.Mvc.Libs;

public class TiknasMvcLibsService : ITiknasMvcLibsService, ITransientDependency
{
    private Task<bool>? _checkLibsTask;

    public virtual void CheckLibs(ApplicationInitializationContext context)
    {
        var options = context.ServiceProvider.GetRequiredService<IOptions<TiknasMvcLibsOptions>>().Value;
        if (options.CheckLibs)
        {
            var app = context.GetApplicationBuilderOrNull();
            if (app == null)
            {
                var logger = context.ServiceProvider.GetRequiredService<ILogger<TiknasMvcLibsService>>();
    ...
    {
        var logger = httpContext.RequestServices.GetRequiredService<ILogger<TiknasMvcLibsService>>();
        try
        {
