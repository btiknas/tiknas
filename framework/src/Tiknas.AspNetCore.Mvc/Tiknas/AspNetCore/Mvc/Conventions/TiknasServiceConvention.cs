...
public class TiknasServiceConvention : ITiknasServiceConvention, ITransientDependency
{
    public ILogger<TiknasServiceConvention> Logger { get; set; }

    protected TiknasAspNetCoreMvcOptions Options { get; }
    protected IConventionalRouteBuilder ConventionalRouteBuilder { get; }

    public TiknasServiceConvention(
        ...
        IConventionalRouteBuilder conventionalRouteBuilder)
    {
        ...
        Logger = NullLogger<TiknasServiceConvention>.Instance;
    }
    ...
    protected virtual void ConfigureSelector(string rootPath, string controllerName, ActionModel action, ConventionalControllerSetting? configuration)
    {
        ...
        {
            AddTiknasServiceSelector(rootPath, controllerName, action, configuration);
        }
        else
        {
    ...
    protected virtual void AddTiknasServiceSelector(string rootPath, string controllerName, ActionModel action, ConventionalControllerSetting? configuration)
    {
        var httpMethod = SelectHttpMethod(action, configuration);

        var tiknasServiceSelectorModel = new SelectorModel
        {
            AttributeRouteModel = CreateTiknasServiceAttributeRouteModel(rootPath, controllerName, action, httpMethod, configuration),
            ActionConstraints = { new HttpMethodActionConstraint(new[] { httpMethod }) }
        };

        action.Selectors.Add(tiknasServiceSelectorModel);
    }
    ...
    protected virtual void NormalizeSelectorRoutes(string rootPath, string controllerName, ActionModel action, ConventionalControllerSetting? configuration)
    {
        foreach (var selector in action.Selectors)
        {
            ...
            {
                selector.AttributeRouteModel = CreateTiknasServiceAttributeRouteModel(rootPath, controllerName, action, httpMethod, configuration);
            }
    ...
    protected virtual AttributeRouteModel CreateTiknasServiceAttributeRouteModel(string rootPath, string controllerName, ActionModel action, string httpMethod, ConventionalControllerSetting? configuration)
    {
