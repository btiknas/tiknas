...
public class EntityHistoryHelper : IEntityHistoryHelper, ITransientDependency
{
    ...
    protected TiknasEfCoreNavigationHelper? TiknasEfCoreNavigationHelper { get; set; }
    ...
    {
        TiknasEfCoreNavigationHelper = tiknasEfCoreNavigationHelper;
    }
    ...
    protected virtual DateTime GetChangeTime(EntityChangeInfo entityChange)
    {
        ...
        switch (entityChange.ChangeType)
        {
            ...
            default:
                throw new TiknasException($"Unknown {nameof(EntityChangeInfo)}: {entityChange}");
        }
    }
    ...
    protected virtual string? GetEntityId(object entityAsObj)
    {
        ...
        throw new TiknasException($"Entities should implement the {typeof(IEntity).AssemblyQualifiedName} interface or {typeof(ValueObject).AssemblyQualifiedName} class! " +
    ...
    protected virtual List<EntityPropertyChangeInfo> GetPropertyChanges(EntityEntry entityEntry)
    {
        ...
        if (Options.SaveEntityHistoryWhenNavigationChanges && TiknasEfCoreNavigationHelper != null)
        {
            foreach (var (navigationEntry, index) in entityEntry.Navigations.Select((value, i) => ( value, i )))
            {
                if (TiknasEfCoreNavigationHelper.IsNavigationEntryModified(entityEntry, index))
                {
                    propertyChanges.Add(new EntityPropertyChangeInfo
                    {
    ...
    protected virtual bool HasNavigationPropertiesChanged(EntityEntry entityEntry)
    {
        return Options.SaveEntityHistoryWhenNavigationChanges &&
               TiknasEfCoreNavigationHelper != null &&
               TiknasEfCoreNavigationHelper.IsEntityEntryModified(entityEntry);
    }
