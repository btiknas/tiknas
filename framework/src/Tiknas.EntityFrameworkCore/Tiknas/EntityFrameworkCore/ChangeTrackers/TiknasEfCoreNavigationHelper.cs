...
/// </summary>
public class TiknasEfCoreNavigationHelper : ITransientDependency
{
    protected Dictionary<string, TiknasEntityEntry> EntityEntries { get; } = new();
    ...
    protected virtual void EntityEntryTrackedOrStateChanged(EntityEntry entityEntry)
    {
        ...
        EntityEntries.Add(entryId, new TiknasEntityEntry(entryId, entityEntry));
    }
    ...
    protected virtual void RecursiveDetectChanges(EntityEntry entityEntry)
    {
        ...
        foreach (var foreignKey in foreignKeys)
        {
            ...
            if (entryId == null || !EntityEntries.TryGetValue(entryId, out var tiknasEntityEntry))
            {
                continue;
            }

            if (!tiknasEntityEntry.IsModified)
            {
                tiknasEntityEntry.IsModified = true;
                RecursiveDetectChanges(tiknasEntityEntry.EntityEntry);
            }

            var navigationEntry = tiknasEntityEntry.NavigationEntries.FirstOrDefault(x => x.NavigationEntry.Metadata is INavigation navigationMetadata && navigationMetadata.ForeignKey == foreignKey) ??
                                  tiknasEntityEntry.NavigationEntries.FirstOrDefault(x => x.NavigationEntry.Metadata is ISkipNavigation skipNavigationMetadata && skipNavigationMetadata.ForeignKey == foreignKey);
        ...
        foreach (var skipNavigation in skipNavigations)
        {
            ...
            foreach (var joinEntry in stateManager.Entries)
            {
                ...
                var navigationEntry = tiknasEntityEntry.NavigationEntries.FirstOrDefault(x => x.NavigationEntry.Metadata is INavigation navigationMetadata && navigationMetadata.ForeignKey == inverseForeignKey) ??
                                      tiknasEntityEntry.NavigationEntries.FirstOrDefault(x => x.NavigationEntry.Metadata is ISkipNavigation skipNavigationMetadata && skipNavigationMetadata.ForeignKey == inverseForeignKey);
                if (navigationEntry != null)
                {
                    navigationEntry.IsModified = true;
                }
...
#pragma warning restore EF1001
    ...
    public virtual bool IsEntityEntryModified(EntityEntry entityEntry)
    {
        ...
        return EntityEntries.TryGetValue(entryId, out var tiknasEntityEntry) && tiknasEntityEntry.IsModified;
    }
    ...
    public virtual bool IsNavigationEntryModified(EntityEntry entityEntry, int? navigationEntryIndex = null)
    {
        ...
        if (!EntityEntries.TryGetValue(entryId, out var tiknasEntityEntry))
        {
            return false;
        }
        ...
        {
            return tiknasEntityEntry.NavigationEntries.Any(x => x.IsModified);
        }

        var navigationEntryProperty = tiknasEntityEntry.NavigationEntries.ElementAtOrDefault(navigationEntryIndex.Value);
