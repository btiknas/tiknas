...
public class TiknasStringToEnumConverter<T> : JsonConverter<T>
    where T : struct, Enum
{
    ...
    public TiknasStringToEnumConverter()
        : this(namingPolicy: null, allowIntegerValues: true)
    {

    }

    public TiknasStringToEnumConverter(JsonNamingPolicy? namingPolicy = null, bool allowIntegerValues = true)
    {
    ...
    public override T Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        _readJsonSerializerOptions ??= JsonSerializerOptionsHelper.Create(options, x =>
                x == this ||
                x.GetType() == typeof(TiknasStringToEnumFactory),
