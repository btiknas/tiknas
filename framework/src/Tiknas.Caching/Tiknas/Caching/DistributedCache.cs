...
{
    public const string UowCacheName = "TiknasDistributedCache";
    ...
    private readonly TiknasDistributedCacheOptions _distributedCacheOption;

    public DistributedCache(
        IOptions<TiknasDistributedCacheOptions> distributedCacheOption,
    ...
    protected virtual KeyValuePair<TCacheKey, TCacheItem?>[] ToCacheItems(byte[]?[] itemBytes, TCacheKey[] itemKeys)
    {
        ...
        {
            throw new TiknasException("count of the item bytes should be same with the count of the given keys");
        }
    ...
    protected virtual Dictionary<TCacheKey, UnitOfWorkCacheItem<TCacheItem>> GetUnitOfWorkCache()
    {
        ...
        {
            throw new TiknasException($"There is no active UOW.");
        }
