...
public class TiknasPopoverTagHelperService : TiknasTagHelperService<TiknasPopoverTagHelper>
{
    protected IHtmlGenerator HtmlGenerator { get; }

    public TiknasPopoverTagHelperService(IHtmlGenerator htmlGenerator)
    {
    ...
    protected virtual void SetDisabled(TagHelperContext context, TagHelperOutput output)
    {
        ...
        {
            //dataPlacementAsHtml = string.Empty; //bootstrap default placement is right, tiknas's is top.
    ...
    protected virtual string GetDataContent()
    {
        switch (GetDirectory())
        {
            case PopoverDirectory.Top:
                return TagHelper.TiknasPopoverTop!;
            case PopoverDirectory.Right:
                return TagHelper.TiknasPopoverRight!;
            case PopoverDirectory.Bottom:
                return TagHelper.TiknasPopoverBottom!;
            case PopoverDirectory.Left:
                return TagHelper.TiknasPopoverLeft!;
            default:
                return TagHelper.TiknasPopover!;
        }
    }

    protected virtual PopoverDirectory GetDirectory()
    {
        if (!string.IsNullOrWhiteSpace(TagHelper.TiknasPopoverTop))
        {
            return PopoverDirectory.Top;
        }
        if (!string.IsNullOrWhiteSpace(TagHelper.TiknasPopoverBottom))
        {
            return PopoverDirectory.Bottom;
        }
        if (!string.IsNullOrWhiteSpace(TagHelper.TiknasPopoverRight))
        {
            return PopoverDirectory.Right;
        }
        if (!string.IsNullOrWhiteSpace(TagHelper.TiknasPopoverLeft))
        {
