...
public class TiknasTabsTagHelperService : TiknasTagHelperService<TiknasTabsTagHelper>
{
    protected IHtmlGenerator HtmlGenerator { get; }

    public TiknasTabsTagHelperService(IHtmlGenerator htmlGenerator)
    {
    ...
    protected virtual void SetActiveTab(List<TabItem> items)
    {
        ...
        foreach (var tabItem in items)
        {
            if (tabItem.Active)
            {
                tabItem.Content = tabItem.Content.Replace(TiknasTabItemShowActivePlaceholder, " show active");
                tabItem.Header = tabItem.Header.Replace(TiknasTabItemActivePlaceholder, " active").Replace(TiknasTabItemSelectedPlaceholder, "true");
            }
            else
            {
                tabItem.Content = tabItem.Content.Replace(TiknasTabItemShowActivePlaceholder, "");
                tabItem.Header = tabItem.Header.Replace(TiknasTabItemActivePlaceholder, "").Replace(TiknasTabItemSelectedPlaceholder, "false");
            }
        }
    ...
    protected virtual string GetHeaders(TagHelperContext context, TagHelperOutput output, List<TabItem> items)
    {
        ...
        for (var index = 0; index < items.Count; index++)
        {
            ...
            if (item.IsDropdown)
            {
                ...
                header = item.Header.Replace(TiknasTabDropdownItemsActivePlaceholder, childHeadersAsString);
            }
