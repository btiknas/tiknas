...
public class TiknasDropdownButtonTagHelperService : TiknasTagHelperService<TiknasDropdownButtonTagHelper>
{

    private readonly HtmlEncoder _htmlEncoder;
    private readonly IServiceProvider _serviceProvider;

    public TiknasDropdownButtonTagHelperService(
    ...
    protected virtual async Task<string> GetMainButtonAsync(TagHelperContext context, TagHelperOutput output, TagHelperContent content)
    {
        ...
        tiknasButtonTagHelper.Icon = TagHelper.Icon;
        tiknasButtonTagHelper.Text = TagHelper.Text;
        tiknasButtonTagHelper.IconType = TagHelper.IconType;
        tiknasButtonTagHelper.Size = TagHelper.Size;
        tiknasButtonTagHelper.ButtonType = TagHelper.ButtonType;
        var attributes = GetAttributesForMainButton(context, output);

        var buttonTag = await tiknasButtonTagHelper.ProcessAndGetOutputAsync(attributes, context, "button", TagMode.StartTagAndEndTag);
    ...
    protected virtual async Task<string> GetSplitButtonAsync(TagHelperContext context, TagHelperOutput output)
    {
        ...
        return await tiknasButtonTagHelper.RenderAsync(attributes, context, _htmlEncoder, "button", TagMode.StartTagAndEndTag);
    }
