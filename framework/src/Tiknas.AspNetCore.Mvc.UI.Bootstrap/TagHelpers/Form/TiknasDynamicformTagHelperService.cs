...
using Tiknas.AspNetCore.Mvc.UI.Bootstrap.TagHelpers.Form.DatePicker;

namespace Tiknas.AspNetCore.Mvc.UI.Bootstrap.TagHelpers.Form;

public class TiknasDynamicFormTagHelperService : TiknasTagHelperService<TiknasDynamicFormTagHelper>
{
    protected HtmlEncoder HtmlEncoder { get; }
    protected IHtmlGenerator HtmlGenerator { get; }
    protected IServiceProvider _serviceProvider { get; }
    protected IStringLocalizer<TiknasUiResource> Localizer { get; }
    protected List<ModelExpression> Models = new();

    public TiknasDynamicFormTagHelperService(
    ...
    protected virtual void SetContent(TagHelperContext context, TagHelperOutput output, List<FormGroupItem> items, TagHelperContent childContent)
    {
        ...
        if (content.Contains(TiknasFormContentPlaceHolder))
        {
            content = content.Replace(TiknasFormContentPlaceHolder, contentBuilder.ToString());
        }
        else
        {
    ...
    {
        var tiknasDateTagHelper = GetDateGroupTagHelper(context, output, model);

        await tiknasDateTagHelper.RenderAsync(new TagHelperAttributeList(), context, HtmlEncoder, "div", TagMode.StartTagAndEndTag);
    }

    protected virtual TiknasTagHelper GetDateGroupTagHelper(TagHelperContext context, TagHelperOutput output, ModelExpression model)
    {
        return IsDateRangeGroup(model.ModelExplorer)
            ? GetTiknasDateRangeInputTagHelper(context, output, model)
            : GetTiknasDateInputTagHelper(model);
    }

    protected virtual TiknasTagHelper GetTiknasDateInputTagHelper(ModelExpression model)
    {
        var tiknasDateInputTagHelper = _serviceProvider.GetRequiredService<TiknasDatePickerTagHelper>();
        tiknasDateInputTagHelper.AspFor = model;
        tiknasDateInputTagHelper.ViewContext = TagHelper.ViewContext;
        tiknasDateInputTagHelper.DisplayRequiredSymbol = TagHelper.RequiredSymbols ?? true;
        return tiknasDateInputTagHelper;
    }

    protected virtual TiknasTagHelper GetTiknasDateRangeInputTagHelper(TagHelperContext context, TagHelperOutput output, ModelExpression model)
    {
        var modelAttribute = model.ModelExplorer.GetAttribute<DateRangePickerAttribute>()!;

        var pickerId = modelAttribute.PickerId;

        var tiknasDateRangeInputTagHelper = _serviceProvider.GetRequiredService<TiknasDateRangePickerTagHelper>();
        tiknasDateRangeInputTagHelper.PickerId = pickerId;
        tiknasDateRangeInputTagHelper.ViewContext = TagHelper.ViewContext;

        if (modelAttribute.IsStart)
        {
            tiknasDateRangeInputTagHelper.AspForStart = model;
            ...
            {
                tiknasDateRangeInputTagHelper.AspForEnd = ModelExplorerToModelExpressionConverter(otherModel!);
            }
        }

        return tiknasDateRangeInputTagHelper;
    }
    ...
    {
        var tiknasSelectTagHelper = GetSelectGroupTagHelper(context, output, model);

        await tiknasSelectTagHelper.RenderAsync(new TagHelperAttributeList(), context, HtmlEncoder, "div", TagMode.StartTagAndEndTag);
    }

    protected virtual TiknasTagHelper GetSelectGroupTagHelper(TagHelperContext context, TagHelperOutput output, ModelExpression model)
    {
        return IsRadioGroup(model.ModelExplorer) ?
            GetTiknasRadioInputTagHelper(model) :
            GetSelectTagHelper(model);
    }

    protected virtual TiknasTagHelper GetSelectTagHelper(ModelExpression model)
    {
        var tiknasSelectTagHelper = _serviceProvider.GetRequiredService<TiknasSelectTagHelper>();
        tiknasSelectTagHelper.AspFor = model;
        tiknasSelectTagHelper.AspItems = null;
        tiknasSelectTagHelper.ViewContext = TagHelper.ViewContext;
        return tiknasSelectTagHelper;
    }

    protected virtual TiknasTagHelper GetTiknasRadioInputTagHelper(ModelExpression model)
    {
        var radioButtonAttribute = model.ModelExplorer.GetAttribute<TiknasRadioButton>()!;
        var tiknasRadioInputTagHelper = _serviceProvider.GetRequiredService<TiknasRadioInputTagHelper>();
        tiknasRadioInputTagHelper.AspFor = model;
        tiknasRadioInputTagHelper.AspItems = null;
        tiknasRadioInputTagHelper.Inline = radioButtonAttribute.Inline;
        tiknasRadioInputTagHelper.Disabled = radioButtonAttribute.Disabled;
        tiknasRadioInputTagHelper.ViewContext = TagHelper.ViewContext;
        return tiknasRadioInputTagHelper;
    }
    ...
    {
        var tiknasButtonTagHelper = _serviceProvider.GetRequiredService<TiknasButtonTagHelper>();
        var attributes = new TagHelperAttributeList { new TagHelperAttribute("type", "submit") };
        tiknasButtonTagHelper.Text = Localizer["Submit"];
        tiknasButtonTagHelper.ButtonType = TiknasButtonType.Primary;

        return await tiknasButtonTagHelper.RenderAsync(attributes, context, HtmlEncoder, "button", TagMode.StartTagAndEndTag);
    }
    ...
    {
        var tiknasInputTagHelper = _serviceProvider.GetRequiredService<TiknasInputTagHelper>();
        tiknasInputTagHelper.AspFor = model;
        tiknasInputTagHelper.ViewContext = TagHelper.ViewContext;
        tiknasInputTagHelper.DisplayRequiredSymbol = TagHelper.RequiredSymbols ?? true;

        await tiknasInputTagHelper.RenderAsync(new TagHelperAttributeList(), context, HtmlEncoder, "div", TagMode.StartTagAndEndTag);
    }
    ...
    {
        return explorer.GetAttribute<TiknasRadioButton>() != null;
    }
}
