...
public abstract class TiknasDatePickerBaseTagHelperService<TTagHelper> : TiknasTagHelperService<TTagHelper>
    where TTagHelper : TiknasDatePickerBaseTagHelper<TTagHelper>
{
    ...
    protected readonly ITiknasTagHelperLocalizer TagHelperLocalizer;
    protected virtual string TagName { get; set; } = "tiknas-date-picker";
    protected IStringLocalizer<TiknasUiResource> L { get; }
    protected abstract TagHelperOutput TagHelperOutput { get; set; }

    protected TiknasDatePickerBaseTagHelperService(IJsonSerializer jsonSerializer, IHtmlGenerator generator,
        HtmlEncoder encoder, IServiceProvider serviceProvider, IStringLocalizer<TiknasUiResource> l,
        ITiknasTagHelperLocalizer tagHelperLocalizer)
    {
    ...
    public async override Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
    {
        ...
        var tiknasDatePickerTag = new TagHelperOutput(TagName, GetBaseTagAttributes(context, output, TagHelper),
            (_, _) => Task.FromResult<TagHelperContent>(new DefaultTagHelperContent()));
        tiknasDatePickerTag.Content.AppendHtml(inputGroup.Render(Encoder));
        tiknasDatePickerTag.Content.AppendHtml(validationContent);
        tiknasDatePickerTag.Content.AppendHtml(GetExtraInputHtml(context, output));

        var innerHtml = labelContent + tiknasDatePickerTag.Render(Encoder) + infoContent;
    ...
    protected TagHelperAttributeList ConvertDatePickerOptionsToAttributeList(ITiknasDatePickerOptions? options)
    {
        var attrList = new TagHelperAttributeList();
        ...
        {
            case TiknasDatePickerWeekNumbers.Normal:
                attrList.Add("data-show-week-numbers", "true");
                break;
            case TiknasDatePickerWeekNumbers.Iso:
        ...
        if (options.Opens != TiknasDatePickerOpens.Center)
        {
            attrList.Add("data-opens", options.Opens.ToString().ToLowerInvariant());
        }

        if (options.Drops != TiknasDatePickerDrops.Down)
        {
    ...
    protected TagHelperAttributeList GetBaseTagAttributes(TagHelperContext context, TagHelperOutput output, ITiknasDatePickerOptions options)
    {
        var groupPrefix = "group-";
    ...
    {
        var tiknasButtonTagHelper = ServiceProvider.GetRequiredService<TiknasButtonTagHelper>();
        var attributes =
            new TagHelperAttributeList { new("type", "button"), new("tabindex", "-1"), new("data-type", type) };
        tiknasButtonTagHelper.ButtonType = TiknasButtonType.Outline_Secondary;
        tiknasButtonTagHelper.Icon = icon;

        tiknasButtonTagHelper.Disabled = TagHelper.IsDisabled || GetAttribute<DisabledInput>() != null;
        ...
        return await tiknasButtonTagHelper.RenderAsync(attributes, context, Encoder, "button", TagMode.StartTagAndEndTag);
    }
    ...
    protected virtual string GetSize(TagHelperContext context, TagHelperOutput output)
    {
        ...
        return TagHelper.Size switch {
            TiknasFormControlSize.Small => "form-control-sm",
            TiknasFormControlSize.Medium => "form-control-md",
            TiknasFormControlSize.Large => "form-control-lg",
            _ => ""
        };
    }
