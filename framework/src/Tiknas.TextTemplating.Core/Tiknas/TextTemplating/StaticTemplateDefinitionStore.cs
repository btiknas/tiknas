...
public class StaticTemplateDefinitionStore : IStaticTemplateDefinitionStore, ISingletonDependency
{
    ...
    public StaticTemplateDefinitionStore(IOptions<TiknasTextTemplatingOptions> options, IServiceProvider serviceProvider)
    {
    ...
    public virtual Task<TemplateDefinition> GetAsync(string name)
    {
        ...
        if (template == null)
        {
            throw new TiknasException("Undefined template: " + name);
        }
