...
public class DefaultTiknasAuthorizationExceptionHandler : ITiknasAuthorizationExceptionHandler, ITransientDependency
{
    public virtual async Task HandleAsync(TiknasAuthorizationException exception, HttpContext httpContext)
    {
        var handlerOptions = httpContext.RequestServices.GetRequiredService<IOptions<TiknasAuthorizationExceptionHandlerOptions>>().Value;
        ...
        if (!handlerOptions.AuthenticationScheme.IsNullOrWhiteSpace())
        {
            ...
            if (scheme == null)
            {
                throw new TiknasException($"No authentication scheme named {handlerOptions.AuthenticationScheme} was found.");
            }
        }
        else
        {
            if (isAuthenticated)
            {
                ...
                if (scheme == null)
                {
                    throw new TiknasException($"There was no DefaultForbidScheme found.");
                }
            }
            else
            {
                ...
                if (scheme == null)
                {
                    throw new TiknasException($"There was no DefaultChallengeScheme found.");
                }
            }
        }
        ...
        if (handler == null)
        {
            throw new TiknasException($"No handler of {scheme.Name} was found.");
        }
