...
public class PermissionDefinitionContext : IPermissionDefinitionContext
{
        ...
        ILocalizableString? displayName = null)
    {
        ...
        {
            throw new TiknasException($"There is already an existing permission group with name: {name}");
        }
    ...
    public virtual PermissionGroupDefinition GetGroup([NotNull] string name)
    {
        ...
        if (group == null)
        {
            throw new TiknasException($"Could not find a permission definition group with the given name: {name}");
        }
    ...
    public virtual void RemoveGroup(string name)
    {
        ...
        if (!Groups.ContainsKey(name))
        {
            throw new TiknasException($"Not found permission group with name: {name}");
        }
