...
public class BackgroundJobExecuter : IBackgroundJobExecuter, ITransientDependency
{
    ...
    public BackgroundJobExecuter(IOptions<TiknasBackgroundJobOptions> options, ICurrentTenant currentTenant)
    {
    ...
    public virtual async Task ExecuteAsync(JobExecutionContext context)
    {
        ...
        if (job == null)
        {
            throw new TiknasException("The job type is not registered to DI: " + context.JobType);
        }
        ...
        {
            throw new TiknasException($"Given job type does not implement {typeof(IBackgroundJob<>).Name} or {typeof(IAsyncBackgroundJob<>).Name}. " +
