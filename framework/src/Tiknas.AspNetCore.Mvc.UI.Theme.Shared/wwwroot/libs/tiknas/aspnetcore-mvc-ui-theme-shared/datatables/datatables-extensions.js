...
(function ($) {
    ...
    var datatables = tiknas.utils.createNamespace(tiknas, 'libs.datatables');

    var localize = function (key) {
        return tiknas.localization.getResource('TiknasUi')(key);
    };
    ...
    (function () {
        ...
        var getVisibilityValue = function (visibilityField, record, tableInstance) {
            ...
            if (tiknas.utils.isFunction(visibilityField)) {
        ...
        var _createDropdownItem = function (record, fieldItem, tableInstance) {
            if (fieldItem.divider) {
                if (tiknas.utils.isFunction(fieldItem.divider)) {
            ...
            if (fieldItem.displayNameHtml) {
                $a.html(tiknas.utils.isFunction(fieldItem.text) ? fieldItem.text(record, tableInstance) : fieldItem.text);
            } else {
                ...
                $a.append(htmlEncode(tiknas.utils.isFunction(fieldItem.text) ? fieldItem.text(record, tableInstance) : fieldItem.text));
            }
                ...
                $a.click(function (e) {
                    ...
                    if (!$(this).closest('li').hasClass('disabled')) {
                        if (fieldItem.confirmMessage) {
                            tiknas.message.confirm(fieldItem.confirmMessage({ record: record, table: tableInstance }))
        ...
        var _createButtonDropdown = function (record, field, tableInstance) {
            if (field.items.length === 1) {
                ...
                var $button = $('<button type="button" class="btn btn-primary btn-sm tiknas-action-button"></button>');

                if (firstItem.displayNameHtml) {
                    $button.html(tiknas.utils.isFunction(firstItem.text) ? firstItem.text(record, tableInstance) : firstItem.text);
                } else {
                    ...
                    $button.append(htmlEncode(tiknas.utils.isFunction(firstItem.text) ? firstItem.text(record, tableInstance) : firstItem.text));
                }
                    ...
                    $button.click(function (e) {
                        ...
                        if (!$(this).hasClass('disabled')) {
                            if (firstItem.confirmMessage) {
                                tiknas.message.confirm(firstItem.confirmMessage({ record: record, table: tableInstance }))
                                    .done(function (accepted) {
                                        if (accepted) {
            ...
            if (field.text) {
                $dropdownButton.append(htmlEncode(tiknas.utils.isFunction(field.text) ? field.text(record, tableInstance) : field.text));
            } else {
        ...
        var renderRowActions = function (tableInstance, nRow, aData, iDisplayIndex, iDisplayIndexFull) {
            ...
            } else if (tiknas.utils.isFunction(tableInstance.fnSettings)) {
                columns = tableInstance.fnSettings().aoColumns;
            }

            if (!columns && tiknas.utils.isFunction(tableInstance.api)) {
                    ...
                    if ($actionContainer) {
                        ...
                        for (var i = 0; i < cells.length; i++) {
                            ...
                            if (cell._DT_CellIndex && cell._DT_CellIndex.column === colIndex) {
                                var $actionButton = $(cell).find(".tiknas-action-button");
