(function ($) {
    ...
    tiknas.libs.bootstrapDateRangePicker = {
        ...
        createDatePicker: function (options) {
            ...
            var $datePicker = options.singleDatePicker ? $('<tiknas-date-picker></tiknas-date-picker>') : $('<tiknas-date-range-picker></tiknas-date-range-picker>');
            ...
            tiknas.dom.initializers.initializeDateRangePickers($datePicker);
    ...
    function isEmptyDate(date, options) {
        ...
        return !TiknasDate(date, options).isValid();
    }

    function TiknasDate(originalDate, options, format) {

        var _moment = convertToMoment(originalDate, options, format);
        function _clone() {
            return TiknasDate(_moment, options, format);
        }

        function _isSame(date) {
            return _format(options.inputDateFormat) === TiknasDate(date, options).format(options.inputDateFormat);
        }
    ...
    function setOptions(options, $datePickerElement, singleDatePicker) {
        ...
        var defaultOptions = {
            ...
            locale: {
                direction: tiknas.localization.currentCulture.isRightToLeft ? 'rtl' : 'ltr',
                todayLabel: tiknas.localization.localize('Today', 'TiknasUi'),
                clearLabel: tiknas.localization.localize('Clear', 'TiknasUi'),
                applyLabel: tiknas.localization.localize('Apply', 'TiknasUi'),
            },
        ...
        if (options.ranges) {
            $.each(options.ranges, function (key, value) {
                ...
                options.ranges[key] = [TiknasDate(start, options), TiknasDate(end, options)];
            });
        }
    ...
    function createInitialTiknasDate(date, options) {
        date = convertToMoment(date, options, undefined, options.isUtc);
        if (options.isUtc) {
            date = date.local();
        }

        return TiknasDate(date, options);
    }

    tiknas.dom.initializers.initializeDateRangePickers = function ($rootElement) {
        $rootElement
            .findWithSelf('tiknas-date-picker,tiknas-date-range-picker')
            .each(function () {
                var $this = $(this);
                ...
                var singleDatePicker = $this.is('tiknas-date-picker')
                var options = {};
                ...
                var startDate = createInitialTiknasDate(options.startDate || options.date || (options.autoUpdateInput ? new Date() : undefined), options);

                var oldStartDate = TiknasDate(undefined, options);

                var endDate = createInitialTiknasDate(options.endDate || (options.autoUpdateInput ? new Date() : undefined), options);
                var oldEndDate = TiknasDate(undefined, options);
                ...
                function setDataDates(date, $selfInput, prefix){
                    date = TiknasDate(date, options).date;
                ...
                function setStartDateByMomentDate (momentDate) {
                    ...
                    startDate = TiknasDate(momentDate, options);
                }
                ...
                function setEndDateByMomentEndDate(momentDate){
                    ...
                    oldEndDate = endDate;
                    endDate = TiknasDate(momentDate, options);
                }
    ...
    tiknas.dom.onNodeAdded(function (args) {
        tiknas.dom.initializers.initializeDateRangePickers(args.$el);
    });

    $(function () {
        tiknas.dom.initializers.initializeDateRangePickers($('body'));
    });
})(jQuery);