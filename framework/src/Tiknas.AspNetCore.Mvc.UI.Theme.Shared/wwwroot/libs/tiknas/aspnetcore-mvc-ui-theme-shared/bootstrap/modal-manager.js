...
(function ($) {
    ...
    tiknas.modals = tiknas.modals || {};

    tiknas.ModalManager = (function () {
        ...
        return function (options) {
            ...
            var _modalId = 'Tiknas_Modal_' + (Math.floor((Math.random() * 1000000))) + new Date().getTime();
            ...
            function _createContainer() {
                ...
                var existsModals = $('[id^="Tiknas_Modal_"]');
            ...
            function _initAndShowModal() {
                ...
                var modalClass = tiknas.modals[options.modalClass];
                if (modalClass) {
                ...
                _createContainer(_modalId)
                    .load(options.viewUrl, $.param(argsWithoutFunc), function (response, status, xhr) {
                        if (status === "error") {
                            ...
                            tiknas.ajax.showError(responseJSON.error ? responseJSON.error : tiknas.ajax.defaultError);
                            return;
                        };

                        if (options.scriptUrl) {
                            tiknas.ResourceLoader.loadScript(options.scriptUrl, function () {
                                _initAndShowModal();
                            });
                        } else {
                            _initAndShowModal();
