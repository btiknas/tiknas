...
{
    protected TiknasRabbitMqEventBusOptions TiknasRabbitMqEventBusOptions { get; }
    ...
    public RabbitMqDistributedEventBus(
        IOptions<TiknasRabbitMqEventBusOptions> options,
        IConnectionPool connectionPool,
        IRabbitMqSerializer serializer,
        IServiceScopeFactory serviceScopeFactory,
        IOptions<TiknasDistributedEventBusOptions> distributedEventBusOptions,
            ...
            correlationIdProvider)
    {
        ...
        TiknasRabbitMqEventBusOptions = options.Value;
    ...
    public void Initialize()
    {
        Consumer = MessageConsumerFactory.Create(
            new ExchangeDeclareConfiguration(
                TiknasRabbitMqEventBusOptions.ExchangeName,
                type: TiknasRabbitMqEventBusOptions.GetExchangeTypeOrDefault(),
                durable: true,
                arguments: TiknasRabbitMqEventBusOptions.ExchangeArguments
            ),
            new QueueDeclareConfiguration(
                TiknasRabbitMqEventBusOptions.ClientName,
                durable: true,
                exclusive: false,
                autoDelete: false,
                prefetchCount: TiknasRabbitMqEventBusOptions.PrefetchCount,
                arguments: TiknasRabbitMqEventBusOptions.QueueArguments
            ),
            TiknasRabbitMqEventBusOptions.ConnectionName
        );

        Consumer.OnMessageReceived(ProcessEventAsync);

        SubscribeHandlers(TiknasDistributedEventBusOptions.Handlers);
    }
    ...
    public async override Task PublishManyFromOutboxAsync(
        ...
        OutboxConfig outboxConfig)
    {
        using (var channel = ConnectionPool.Get(TiknasRabbitMqEventBusOptions.ConnectionName).CreateModel())
        {
        ...
        string? correlationId = null)
    {
        ...
        channel.BasicPublish(
            exchange: TiknasRabbitMqEventBusOptions.ExchangeName,
    ...
    private void EnsureExchangeExists(IModel channel)
    {
        ...
        try
        {
            using (var temporaryChannel = ConnectionPool.Get(TiknasRabbitMqEventBusOptions.ConnectionName).CreateModel())
            {
                temporaryChannel.ExchangeDeclarePassive(TiknasRabbitMqEventBusOptions.ExchangeName);
            }
        }
        catch (Exception)
        {
            channel.ExchangeDeclare(
                TiknasRabbitMqEventBusOptions.ExchangeName,
                TiknasRabbitMqEventBusOptions.GetExchangeTypeOrDefault(),
                durable: true
            );
        }
