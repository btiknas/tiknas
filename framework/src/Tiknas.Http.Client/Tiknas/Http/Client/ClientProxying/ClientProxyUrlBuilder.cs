...
public class ClientProxyUrlBuilder : ITransientDependency
{
    ...
    protected TiknasHttpClientProxyingOptions HttpClientProxyingOptions { get; }

    public ClientProxyUrlBuilder(IServiceScopeFactory serviceScopeFactory, IOptions<TiknasHttpClientProxyingOptions> httpClientProxyingOptions)
    {
    ...
    protected virtual async Task ReplacePathVariablesAsync(StringBuilder urlBuilder, ActionApiDescriptionModel action, IReadOnlyDictionary<string, object> methodArguments, ApiVersionInfo apiVersion)
    {
        ...
        foreach (var pathParameter in pathParameters.Where(p => p.Name != "apiVersion")) //TODO: Constant!
        {
            ...
            if (value == null)
            {
                ...
                {
                    throw new TiknasException($"Missing path parameter value for {pathParameter.Name} ({pathParameter.NameOnMethod})");
                }
            }
            else
            {
