...
using Tiknas.Http.ProxyScripting.Generators;
using Tiknas.Json;
...
public class ClientProxyBase<TService> : ITransientDependency
{
    ...
    protected IOptions<TiknasCorrelationIdOptions> TiknasCorrelationIdOptions => LazyServiceProvider.LazyGetRequiredService<IOptions<TiknasCorrelationIdOptions>>();
    ...
    protected IOptions<TiknasHttpClientOptions> ClientOptions => LazyServiceProvider.LazyGetRequiredService<IOptions<TiknasHttpClientOptions>>();
    ...
    protected virtual ClientProxyRequestContext BuildHttpProxyClientProxyContext(string methodName, ClientProxyRequestTypeValue? arguments = null)
    {
        ...
        if (action == null)
        {
            throw new TiknasException($"The API description of the {typeof(TService).FullName}.{methodName} method was not found!");
        }
    ...
    {
        var clientConfig = ClientOptions.Value.HttpClientProxies.GetOrDefault(requestContext.ServiceType) ?? throw new TiknasException($"Could not get HttpClientProxyConfig for {requestContext.ServiceType.FullName}.");
        ...
        {
            throw new TiknasRemoteCallException($"An error occurred during the TIKNAS remote HTTP request. ({ex.Message}) See the inner exception for details.", ex);
        }
    ...
    protected virtual async Task<string?> GetConfiguredApiVersionAsync(ClientProxyRequestContext requestContext)
    {
        var clientConfig = ClientOptions.Value.HttpClientProxies.GetOrDefault(requestContext.ServiceType)
                           ?? throw new TiknasException($"Could not get DynamicHttpClientProxyConfig for {requestContext.ServiceType.FullName}.");
    ...
    protected virtual async Task ThrowExceptionForResponseAsync(HttpResponseMessage response)
    {
        ...
        if (response.Headers.Contains(TiknasHttpConsts.TiknasErrorFormat))
        {
            ...
            {
                throw new TiknasRemoteCallException(
            ...
            throw new TiknasRemoteCallException(errorResponse.Error)
            {
                HttpStatusCode = (int)response.StatusCode
            };
        }
        ...
        ApiVersionInfo apiVersion)
    {
        ...
        //CorrelationId
        requestMessage.Headers.Add(TiknasCorrelationIdOptions.Value.HttpHeaderName, CorrelationIdProvider.Get());
