...
public static class ServiceCollectionHttpClientProxyExtensions
{
    ...
    /// See <see cref="TiknasRemoteServiceOptions"/>.
    /// </param>
        ...
        ApplicationServiceTypes applicationServiceTypes = ApplicationServiceTypes.All)
    {
        ...
        foreach (var serviceType in serviceTypes)
        {
            ...
            services.Configure<TiknasHttpClientOptions>(options =>
            {
        ...
        bool asDefaultService = true)
    {
        ...
        var interceptorAdapterType = typeof(TiknasAsyncDeterminationInterceptor<>).MakeGenericType(interceptorType);

        var validationInterceptorAdapterType =
            typeof(TiknasAsyncDeterminationInterceptor<>).MakeGenericType(typeof(ValidationInterceptor));
    ...
    private static IServiceCollection AddHttpClientFactory(
        ...
        [NotNull] string remoteServiceConfigurationName = RemoteServiceConfigurationDictionary.DefaultName)
    {
        var preOptions = services.ExecutePreConfiguredActions<TiknasHttpClientBuilderOptions>();
        ...
        services.PreConfigure<TiknasHttpClientBuilderOptions>(options =>
        {
