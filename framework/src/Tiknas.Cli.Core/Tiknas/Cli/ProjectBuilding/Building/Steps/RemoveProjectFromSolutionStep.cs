...
public class RemoveProjectFromSolutionStep : ProjectBuildPipelineStep
{
    ...
    public override void Execute(ProjectBuildContext context)
    {
        ...
        RemoveProjectFromTiknasmdlFile(context);
    }

    private void RemoveProjectFromTiknasmdlFile(ProjectBuildContext context)
    {
        var tiknasmdlFile = context.FindFile(_solutionFilePath.RemovePostFix(".sln") + ".tiknasmdl");

        if (tiknasmdlFile == null)
        {
            return;
        }

        var jsonRoot = JObject.Parse(tiknasmdlFile.Content);
        var packagesObj = (JObject)jsonRoot["packages"];

        packagesObj?.Remove(_projectName);

        tiknasmdlFile.SetContent(jsonRoot.ToString(Formatting.Indented));
    }
