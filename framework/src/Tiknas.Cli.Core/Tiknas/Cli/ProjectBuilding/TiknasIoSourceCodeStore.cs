...
public class TiknasIoSourceCodeStore : ISourceCodeStore, ITransientDependency
{
    public ILogger<TiknasIoSourceCodeStore> Logger { get; set; }
    ...
    public TiknasIoSourceCodeStore(
        IOptions<TiknasCliOptions> options,
        ...
        CliVersionService cliVersionService)
    {
        ...
        Logger = NullLogger<TiknasIoSourceCodeStore>.Instance;
    }
        ...
        bool trustUserVersion = false)
    {
        ...
        if (version == null)
        {
            if (latestVersion == null)
            {
                ...
                Logger.LogWarning(string.Empty);
                throw new CliUsageException("Use command: tiknas new Acme.BookStore -v version");
            }
        ...
        if (type == SourceCodeTypes.Template)
        {
            ...
            if (outputWarning)
            {
                ...
                Logger.LogWarning($"> dotnet tool uninstall -g tiknas.cli");
                Logger.LogWarning(!templateVersion.IsPrerelease
                    ? $"> dotnet tool install -g tiknas.cli --version \"{templateVersion.Major}.{templateVersion.Minor}.*\""
                    : $"> dotnet tool install -g tiknas.cli --version {templateVersion}");
        ...
        bool includePreReleases = false)
    {
        if (url == null)
        {
            url = $"{CliUrls.WwwTiknasIo}api/download/{type}/get-version/";
        }
    ...
    private async Task<string> GetTemplateNugetVersionAsync(string name, string type, string version)
    {
        ...
        try
        {
            var url = $"{CliUrls.WwwTiknasIo}api/download/{type}/get-nuget-version/";
            var client = _cliHttpClientFactory.CreateClient();
    ...
    {
        var url = $"{CliUrls.WwwTiknasIo}api/download/all-versions?includePreReleases=true";
    ...
    {
        var url = $"{CliUrls.WwwTiknasIo}api/download/{input.Type}/";
