...
public class NewCommand : ProjectCreationCommandBase, IConsoleCommand, ITransientDependency
{
    ...
    public string GetUsageInfo()
    {
        ...
        sb.AppendLine("Usage:");
        sb.AppendLine("");
        sb.AppendLine("  tiknas new <project-name> [options]");
        sb.AppendLine("");
        ...
        sb.AppendLine("-ts|--template-source <template-source>     (your local or network tiknas template source)");
        ...
        sb.AppendLine("--local-framework-ref --tiknas-path <your-local-tiknas-repo-path>  (keeps local references to projects instead of replacing with NuGet package references)");
        sb.AppendLine("-sib|--skip-installing-libs                      (Doesn't run `tiknas install-libs` command after project creation)");
        sb.AppendLine("-sb|--skip-bundling                             (Doesn't run `tiknas bundle` command after Blazor Wasm project creation)");
        sb.AppendLine("-sc|--skip-cache                                (Always download the latest from our server and refresh their templates folder cache)");
        sb.AppendLine("");
        sb.AppendLine("Examples:");
        sb.AppendLine("");
        sb.AppendLine("  tiknas new Acme.BookStore");
        sb.AppendLine("  tiknas new Acme.BookStore --tiered");
        sb.AppendLine("  tiknas new Acme.BookStore -u angular");
        sb.AppendLine("  tiknas new Acme.BookStore -u angular -d mongodb");
        sb.AppendLine("  tiknas new Acme.BookStore -m none");
        sb.AppendLine("  tiknas new Acme.BookStore -m react-native");
        sb.AppendLine("  tiknas new Acme.BookStore -m maui");
        sb.AppendLine("  tiknas new Acme.BookStore -d mongodb");
        sb.AppendLine("  tiknas new Acme.BookStore -d mongodb -o d:\\my-project");
        sb.AppendLine("  tiknas new Acme.BookStore -t module");
        sb.AppendLine("  tiknas new Acme.BookStore -t module --no-ui");
        sb.AppendLine("  tiknas new Acme.BookStore -t console");
        sb.AppendLine("  tiknas new Acme.BookStore -ts \"D:\\localTemplate\\tiknas\"");
        sb.AppendLine("  tiknas new Acme.BookStore -csf false");
        sb.AppendLine("  tiknas new Acme.BookStore --local-framework-ref --tiknas-path \"D:\\github\\tiknas\"");
        sb.AppendLine("  tiknas new Acme.BookStore --dbms mysql");
    ...
    {
        return "Generate a new solution based on the TIKNAS startup templates.";
    }
}
