...
{
    private readonly TiknasCliOptions _options;
    ...
    public async Task ExecuteAsync(CommandLineArgs commandLineArgs)
    {
        ...
        if (version == null)
        {
            if (commandLineArgs.Target.Contains("LeptonX"))
            {
                ...
                if (version.Contains("*"))
                {
                    version = SolutionPackageVersionFinder.FindByDllVersion(solutionFile, "Tiknas.*LeptonX*");
                }
            }
            else
            {
    ...
    public string GetUsageInfo()
    {
        ...
        sb.AppendLine("");
        sb.AppendLine("'add-module' command is used to add a multi-package TIKNAS module to a solution.");
        sb.AppendLine("It should be used in a folder containing a .sln file.");
        sb.AppendLine("");
        sb.AppendLine("Usage:");
        sb.AppendLine("  tiknas add-module <module-name> [options]");
        sb.AppendLine("");
        ...
        sb.AppendLine("  -v|--version <version>                          Specify the version of the module. Default is your project's TIKNAS version.");
        sb.AppendLine("");
    ...
    protected virtual string GetSolutionFile(CommandLineArgs commandLineArgs)
    {
        ...
        {
            throw new CliUsageException("'tiknas add-module' command should be used inside a folder containing a .sln file!");
        }
        ...
        sb.AppendLine($"tiknas add-module {commandLineArgs.Target} -p {foundSolutionFiles[0]}");
