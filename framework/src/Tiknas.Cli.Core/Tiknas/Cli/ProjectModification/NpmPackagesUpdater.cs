...
public class NpmPackagesUpdater : ITransientDependency
{
        ...
        bool includePreRc = false)
    {
        ...
        var tiknasPackages = GetTiknasPackagesFromPackageJson(packageJson);

        if (!tiknasPackages.Any())
        {
            return false;
        }

        foreach (var tiknasPackage in tiknasPackages)
        {
            var updated = await TryUpdatingPackage(filePath, tiknasPackage, includePreviews, includeReleaseCandidates,
    ...
    protected virtual List<JProperty> GetTiknasPackagesFromPackageJson(JObject fileObject)
    {
        var dependencyList = new[] { "dependencies", "devDependencies", "peerDependencies" };
        var tiknasPackages = new List<JProperty>();
