...
public class ProjectNpmPackageAdder : ITransientDependency
{
        ...
        bool skipInstallingLibs = false)
    {
        ...
        if (version == null)
        {
            version = DetectTiknasVersionOrNull(Path.Combine(directory, "package.json"));
        }
    ...
    private string DetectTiknasVersionOrNull(string packageJsonFile)
    {
        ...
        {
            Logger.LogWarning("Cannot detect TIKNAS package version. " + ex.Message);
        }
    ...
    {
        var url = $"{CliUrls.WwwTiknasIo}api/app/npmPackage/byName/?name=" + packageName;
