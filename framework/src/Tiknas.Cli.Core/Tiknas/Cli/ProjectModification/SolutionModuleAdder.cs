...
using Tiknas.Cli.ProjectModification.Events;
...
public class SolutionModuleAdder : ITransientDependency
{
        ...
        bool newProTemplate = false)
    {
        ...
        {
            await SetLeptonXTiknasVersionsAsync(solutionFile, Path.Combine(modulesFolderInSolution, module.Name));
        }
        ...
        NuGetPackageTarget target, bool isTieredProject)
    {
        ...
        {
            if (target == NuGetPackageTarget.Web && package.Name.StartsWith("Tiknas.Account"))
            {
                continue;
            }
    ...
    protected async Task ModifyDbContext(string[] projectFiles, ModuleInfo module, bool skipDbMigrations = false)
    {
        ...
        var dbContextFile = DerivedClassFinder.Find(dbMigrationsProject, "TiknasDbContext").FirstOrDefault();

        if (dbContextFile == null)
        {
            Logger.LogDebug(
                $"{dbMigrationsProject} project doesn't have a class that is derived from \"TiknasDbContext\".");
            return;
        }
        ...
        bool newProTemplate = false)
    {
        ...
        var url = $"{CliUrls.WwwTiknasIo}api/app/module/byNameWithDetails/?name=" + moduleName;
