...
public class ProjectNugetPackageAdder : ITransientDependency
{
        ...
        bool addSourceCodeToSolutionFile = false)
    {
        ...
        if (version == null)
        {
            version = GetTiknasVersionOrNull(projectFile);
        }
        ...
        bool useDotnetCliToInstall)
    {
        ...
        using (DirectoryHelper.ChangeCurrentDirectory(Path.GetDirectoryName(projectFile)))
        {
            ...
            var moduleFiles = ModuleClassFinder.Find(projectFile, "TiknasModule");
            if (moduleFiles.Count == 0)
            {
                throw new CliUsageException(
                    $"Could not find a class derived from TiknasModule in the project {projectFile}");
            }

            if (moduleFiles.Count > 1)
            {
                throw new CliUsageException(
                    $"There are multiple classes derived from TiknasModule in the project {projectFile}: " +
    ...
    protected virtual string GetTiknasVersionOrNull(string projectFile)
    {
        var projectFileContent = File.ReadAllText(projectFile);
    ...
    {
        var url = $"{CliUrls.WwwTiknasIo}api/app/nugetPackage/byName/?name=" + packageName;
