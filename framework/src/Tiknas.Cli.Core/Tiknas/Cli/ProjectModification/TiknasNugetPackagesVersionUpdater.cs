...
public class TiknasNugetPackagesVersionUpdater : ITransientDependency
{
    ...
    public ILogger<TiknasNugetPackagesVersionUpdater> Logger { get; set; }
    public static Encoding DefaultEncoding = Encoding.UTF8;

    public TiknasNugetPackagesVersionUpdater(PackageVersionCheckerService packageVersionCheckerService, MyGetPackageListFinder myGetPackageListFinder)
    {
        _packageVersionCheckerService = packageVersionCheckerService;
        _myGetPackageListFinder = myGetPackageListFinder;
        Logger = NullLogger<TiknasNugetPackagesVersionUpdater>.Instance;
    }
    ...
    public async Task UpdateSolutionAsync(string solutionPath, bool includePreviews = false, bool includeReleaseCandidates = false, bool switchToStable = false, bool checkAll = false, string version = null)
    {
        ...
        {
            var latestVersionInfo = await _packageVersionCheckerService.GetLatestVersionOrNullAsync("Tiknas.Core", includeReleaseCandidates: includeReleaseCandidates);
            var latestReleaseCandidateVersionInfo = await _packageVersionCheckerService.GetLatestVersionOrNullAsync("Tiknas.Core", includeReleaseCandidates: true);
            var latestVersionFromMyGet = await GetLatestVersionFromMyGet("Tiknas.Core");
    ...
    public async Task UpdateProjectAsync(string projectPath, bool includeNightlyPreviews = false, bool includeReleaseCandidates = false, bool switchToStable = false, bool checkAll = false, string version = null)
    {
        ...
        {
            var latestVersionInfo = await _packageVersionCheckerService.GetLatestVersionOrNullAsync("Tiknas.Core");
            var latestReleaseCandidateVersionInfo = await _packageVersionCheckerService.GetLatestVersionOrNullAsync("Tiknas.Core", includeReleaseCandidates: true);
            var latestVersionFromMyGet = await GetLatestVersionFromMyGet("Tiknas.Core");
        ...
        try
        {
            ...
            if (packageNodeList != null)
            {
                foreach (XmlNode package in packageNodeList)
                {
                    ...
                    var isStudioPackage = packageId.StartsWith("Tiknas.Studio.");
                    if(isLeptonXPackage)
                    {
                    ...
                    else
                    {
                        if ((includeNightlyPreviews || (currentVersion.Contains("-preview") && !switchToStable)) && !includeReleaseCandidates)
                        {
                            ...
                            if(isLeptonXPackage)
                            {
                                ...
                                {
                                    //use LeptonX Lite package as the package name to be able to get the package version from the 'tiknas-nightly' feed.
                                    leptonXPackageName = "Tiknas.AspNetCore.Mvc.UI.Theme.LeptonXLite";
                                }
