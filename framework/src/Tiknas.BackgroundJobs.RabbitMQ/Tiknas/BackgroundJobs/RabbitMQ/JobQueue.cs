...
public class JobQueue<TArgs> : IJobQueue<TArgs>
{
    ...
    protected TiknasRabbitMqBackgroundJobOptions TiknasRabbitMqBackgroundJobOptions { get; }
    ...
    public JobQueue(
        IOptions<TiknasBackgroundJobOptions> backgroundJobOptions,
        IOptions<TiknasRabbitMqBackgroundJobOptions> rabbitMqTiknasBackgroundJobOptions,
    ...
    {
        TiknasBackgroundJobOptions = backgroundJobOptions.Value;
        TiknasRabbitMqBackgroundJobOptions = rabbitMqTiknasBackgroundJobOptions.Value;
        ...
        JobConfiguration = TiknasBackgroundJobOptions.GetJob(typeof(TArgs));
    ...
    {
        return TiknasRabbitMqBackgroundJobOptions.JobQueues.GetOrDefault(typeof(TArgs)) ??
               new JobQueueConfiguration(
                   typeof(TArgs),
                   TiknasRabbitMqBackgroundJobOptions.DefaultQueueNamePrefix + JobConfiguration.JobName,
                   TiknasRabbitMqBackgroundJobOptions.DefaultDelayedQueueNamePrefix + JobConfiguration.JobName,
                   prefetchCount: TiknasRabbitMqBackgroundJobOptions.PrefetchCount
               );
    }
    ...
    public virtual async Task StartAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (!TiknasBackgroundJobOptions.IsJobExecutionEnabled)
        {
            return;
        }
    ...
    protected void CheckDisposed()
    {
        if (IsDiposed)
        {
            throw new TiknasException("This object is disposed!");
        }
    }
}
